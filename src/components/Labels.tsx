...
interface LabelsProps<Value extends string> {
  ...
  function Labels<Value extends string>({ top, domain, rowScale, colorScale, className }: LabelsProps<Value>) {
    return <Group top={top}>
      {domain.map((value) => (
        <Group key={value}
               top={rowScale(value)}
               style={{ color: colorScale(value) }}>
          <CenteredText height={rowScale.bandwidth()} className={className}>{value.toUpperCase()}</CenteredText>
        </Group>
      ))}
    </Group>
  }
